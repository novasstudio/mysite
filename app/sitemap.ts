...
export default function sitemap(): MetadataRoute.Sitemap {
  const base = "https://launchplay.games";
  const routes = ["", "/games", "/about", "/support", "/contact", "/privacy", "/terms"].map(path => ({ url: `${base}${path}`, lastModified: new Date("2026-09-17"), changeFrequency: "monthly" as const, priority: path === "" ? 1 : .7 }));
